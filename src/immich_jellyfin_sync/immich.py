...
class Asset:
    ...
    def syncable(self) -> bool:
        return (self.type == "VIDEO" and self.visibility in SYNCABLE_VISIBILITY
                and not self.is_trashed and not self.is_offline)
...
class ImmichClient:
    ...
    def album_videos(self, album_id: str) -> list[Asset]:
        """All VIDEO assets in an album (unfiltered by visibility; see Asset.syncable)."""
        out: list[Asset] = []
        page = 1
        for _ in range(self.MAX_PAGES):
            data = self._request(
                "POST", "/api/search/metadata",
                json={"albumIds": [album_id], "type": "VIDEO", "page": page, "size": self.PAGE_SIZE},
            )
            ...
            if nxt is None:
                if assets.get("nextCursor"):
                    ...
                    # look like videos were removed from the album).
